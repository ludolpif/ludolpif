...
Mais j'expérimente kitty
```
# apt install kitty
# update-alternatives --config x-terminal-emulator
$ cat >> ~/.config/kitty/kitty.conf <<"EOT"
# My custom config {{{
...
v4l2loopback
EOT
root@lud-5490:~# cat >/etc/modprobe.d/v4l2loopback.conf <<"EOT"
options v4l2loopback devices=1 video_nr=21 exclusive_caps=1 card_label="Virtual Webcam"
EOT
root@lud-5490:~# modprobe v4l2loopback
root@lud-5490:~# ls -l /dev/video21
crw-rw----+ 1 root video 81, 0 20 janv. 12:23 /dev/video21
root@lud-5490:~# uvcdynctrl --clist -d /dev/video21 2> /dev/null
Listing available controls for device /dev/video21:
  keep_format
  sustain_framerate
  timeout
  timeout_image_io
